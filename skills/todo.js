...
module.exports = function(controller) {
    ...
    controller.hears(['tasks','todo'], 'direct_message', function(bot, message) {
        ...
        if (err) bot.reply(message, 'Sorry! Something went wrong. Try again')
        });
    ...
    controller.hears(['add (.*)', 'remember (.*)'],'direct_message,direct_mention,mention', function(bot, message) {
        ...
        controller.storage.users.get(message.user, function(err, user) {
            ...
            if(err) bot.reply(message, 'Sorry! Something went wrong and I couldn\'t add your task :(')
        });
    });
    ...
    controller.hears(['done (.*)'],'direct_message', function(bot, message) {
        ...
        if (isNaN(number)) {
            ...
            controller.storage.users.get(message.user, function(err, user) {
                ...
                if(err) bot.reply(message, 'Sorry, something went wrong and I couldnt mark your task done')
            });
        }
